...
public class CreateMessage extends Message {
    ...
    public CreateMessage(Map<String,Protect> protects) {
    ...
    public static class Handler implements IMessageHandler<CreateMessage,IMessage> {
        @Override
        public IMessage onMessage(CreateMessage message,MessageContext ctx) {
        	FMLLog.info("Create message size: "+message.protects.size());
            ProtectManager.instance().put(message.protects,false);
            return null;
        }
    }
